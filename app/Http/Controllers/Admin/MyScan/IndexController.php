...
class IndexController extends Controller
{
    ...
    public function addFaHuo(Request $request){
         if($request->isMethod('post')){
            ////如果是post过来的数据 那就把发送过来的订单编号添加批次
              ...
              $res = MyScanService::addSellBatch($order_sn,$fa_huo_time,$note);
    ...
    public function printVip(Request $request){
        ...
        $will_print_data = MyScanService::willPrintData($batch_id,$search_con);
...
public function printAllList(Request $request){
    ...
    switch ($print_type){
        case "vip":
            ...
            break;
        case 'A4_vip':
            $print_data = MyScanService::printListVipWithA4($batch_id,$search_con);
            break;
    }
...
public function printList(Request $request){
    ...
    switch ($print_type){
        case "vip":
            ...
            break;
        case 'fashion_list':
            $print_data = MyScanService::printListFashionList($batch_id,$one_code);
            break;
        case 'k_d_list':
            $print_data = MyScanService::printListKD($batch_id,$one_code);
            break;
        case 'A4_vip':
            $print_data = MyScanService::printListVipWithA4($batch_id,$search_con,$one_code);
            break;
...
protected function scaning(Request $request){
    ...
    public function scanList(Request $request){
        ...
        $data = MyScanService::scanList($batch_id);
    ...
    public function printListScan(Request $request){
        ...
        $print_data = MyScanService::printListScan($batch_id,$one_code);
    ...
    public function endScan(Request $request){
        ...
        $need_chu_ku = MyScanService::endScan($batch_id);
 ...
 */
    ...
    public function exportKuaiDi(Request $request){
        ...
        $export_kuai_di_data = MyScanService::exportKuaiDiData($batch_id);
    ...
    public function exportProInfo(Request $request){
        ...
        $export_kuai_di_data = MyScanService::exportProInfo($batch_id);
    ...
    public function editAddress(Request $request){
        ...
        if($request->method()=='POST'){
            ...
            $res = MyScanService::editAddress($address_id,$name,$tel,$province,$city,$area,$detail); ///编辑收货地址
         ...
         $address_info  = MyScanService::getAddressInfo($batch_id,$one_code); ///获取收货地址

        return view('admin.myscan.edit_address',compact('address_info'));
    }
